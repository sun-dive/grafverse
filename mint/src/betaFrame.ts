...
//
// ⚠⚠⚠ WHY A FORK AND NOT A PARAMETER ON THE ORIGINAL. `basicCovenant.ts` is exported by
// `grafbasic.ts` into `vendor/grafbasic.js`, which `basic.html` loads. Adding to it — even additively —
// puts the live BASIC workbench's artefact out of step with its source, and the runbook records
// exactly that failure: *"THE BUNDLE GOES STALE IN SILENCE. On 18 Aug the fold was removed, 47 test
// files went green, and the compiler on the live page went on folding."*
// ⇒ The slot track gets its own frame. `mint/tools/BASIC.md`, and the page-script isolation rule.
//
// ★ WHAT IT ADDS, AND IT IS ONLY THIS: `scope` — so a lane can sign under
// `SIGHASH_ANYONECANPAY | ALL | FORKID` (0xc1) and be funded by a separate battery input, instead of
// paying its own fee out of its own value. → `~/Documents/fuel-depot-spec.md` §7.8.
...
// interchangeable — check that before minting a second lane.
//
// ⚠⚠ THE COST OF A FORK IS DRIFT, SO THE COPY IS PINNED. `test/beta-frame.ts` requires this file to
// produce BYTE-IDENTICAL output to `basicCovenant.ts` at the default scope, across every program it is
// given. Same treatment `racerPhysics.ts` got when the racers forked the shell: duplicate the logic,
// then make the duplicate unable to drift quietly.
/**
...
export interface FrameParams {
  /**
   * ★ The sighash the covenant signs under. Defaults to `SIGHASH_ALL | FORKID` (0x41), which is
   * byte-for-byte what `basicCovenant.ts` produces.
   * ⚠ `0xc1` adds ANYONECANPAY so a SEPARATE input may fund the spend — which is what lets a lane
   * carry no fee bound at all, since it never pays one. It also ZEROES `hashPrevouts`.
   */
  scope?: number
...
/** The default, and what every covenant in this repo signed under before the lane. */
...
export function basicLockOps(p: FrameParams): { ops: ScriptChunk[]; state: StateResult; layout: Field[] } {
  const c = p.c ?? pushTxConstants(p.scope ?? SCOPE)
